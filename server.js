...
app.get('/data/update/water_rights', function(req, res, options){
  ...
  var query = { $and: [ 
    {'kind': 'right'}, 
    {'properties.holder_name': null}, 
    {'properties.diversion_storage_amount':null}]
  };
...
/*
  engine.find_many_by(query,function(error, results) {
    ...
    var output = format_rights(results);
    output += "test";
    res.send(output);
  },{}, {'limit': 1});
*/
...
/*       console.log(obj.features[i].attributes); */
...
watermap_app.formatEWRIMSforSaving = function(feature) {
